Profile: AtMopedClaimLKFRequestGenerisch
Parent: Claim
Id: at-moped-claim-lkfrequest-generisch
Title: "AT MOPED Claim LKFRequest generisches Profil"
...
* procedure.procedure[x] only Reference(AtMopedProcedureBasis)
...
* related[zugehoerigeVAE].claim only Reference(AtMopedClaimVAERequestGenerisch)
...
* related[vorherigerLKFRequest].claim only Reference(AtMopedClaimLKFRequestBasis)
...
* insurance.coverage only Reference(AtMopedCoverageVersicherterBasis)
...
* encounter contains MopedEncounter 1..1 and BewegungsEncounter 1..
* encounter[MopedEncounter] ^short = "Generelle Informationen zu Aufnahme und Entlassung des Patienten"
* encounter[MopedEncounter] only Reference(AtMopedEncounterAufenthaltBesuchGenerisch)
...
* insert MopedHandleObligation(encounter[BewegungsEncounter])

* diagnosis.diagnosis[x] 1..1
* diagnosis.diagnosis[x] only Reference(AtMopedConditionBasis)
...
* insert ShallPopulateObligation(supportingInfo[FinaleAbrechnung], MopedKAActor)
...
* supportingInfo[XDokDevice].value[x] only Reference(AtMopedDeviceXDok)
