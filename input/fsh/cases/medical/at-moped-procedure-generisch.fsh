Profile: AtMopedProcedureGenerisch
Parent: Procedure
...
* encounter only Reference(AtMopedEncounterAufenthaltBesuchGenerisch)
* encounter 1..1
...
* performer.actor only Reference(AtMopedOrganizationOrganisationseinheitKA or AtMopedOrganizationKA)
* performer.actor 1..1
...
* performer 1..1
* performer.onBehalfOf only Reference(AtMopedOrganizationKA)
...
* insert legacyMapping(code, LKF, [[Medizinische Leistung – Code]])
...
Severity: #error
Description: "Wird bei performer.actor eine AtMopedOrganizationKA angegeben so muss sich diese unterscheiden von der referenzierten Krankenanstalt in perfomer.onBehalfOf"
Expression: ""