Instance: Test2OrganizationAbteilung1
InstanceOf: MOPEDOrganizationAbteilung
* id = "Test2OrganizationAbteilung1"
* meta.profile = "http://example.org/StructureDefinition/MOPEDOrganizationAbteilung"
* partOf = Reference(Organization/Test2OrganizationKrankenhausRied)
* identifier[+].type = #Funktionscode
* identifier[=].value = "112111"
* identifier[+].type = #Funktionssubcode
* identifier[=].value = "00"

Instance: Test2OrganizationAbteilung2
InstanceOf: MOPEDOrganizationAbteilung
* id = "Test2OrganizationAbteilung2"
* meta.profile = "http://example.org/StructureDefinition/MOPEDOrganizationAbteilung"
* partOf = Reference(Organization/Test2OrganizationKrankenhausRied)
* identifier[+].type = #Funktionscode
* identifier[=].value = "127181"
* identifier[+].type = #Funktionssubcode
* identifier[=].value = "00"

Instance: Test2OrganizationAbteilung3
InstanceOf: MOPEDOrganizationAbteilung
* id = "Test2OrganizationAbteilung3"
* meta.profile = "http://example.org/StructureDefinition/MOPEDOrganizationAbteilung"
* partOf = Reference(Organization/Test2OrganizationKrankenhausRied)
* identifier[+].type = #Funktionscode
* identifier[=].value = "111111"
* identifier[+].type = #Funktionssubcode
* identifier[=].value = "00"

Instance: Test2OrganizationAbteilung4
InstanceOf: MOPEDOrganizationAbteilung
* id = "Test2OrganizationAbteilung4"
* meta.profile = "http://example.org/StructureDefinition/MOPEDOrganizationAbteilung"
* partOf = Reference(Organization/Test2OrganizationKrankenhausRied)
* identifier[+].type = #Funktionscode
* identifier[=].value = "100000"
* identifier[+].type = #Funktionssubcode
* identifier[=].value = "00"
